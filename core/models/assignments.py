...
class Assignment(db.Model):
    ...
    @classmethod
    def get_assignments_teacher(cls, teacher_id):
        return cls.filter(cls.teacher_id == teacher_id).all()

    @classmethod
    def grade_assignment(cls, assignment_grade, p):
        
        assignment = Assignment.get_by_id(assignment_grade.id)
        assertions.assert_found(assignment, 'No assignment with this id was found')

        assertions.assert_valid(assignment.teacher_id==p.teacher_id, "cannot grade the assignment submited to another teacher")
        assertions.assert_valid(assignment.state==AssignmentStateEnum.SUBMITTED,"only a submitted assignment can be graded")
        
        assignment.grade = assignment_grade.grade
        db.session.flush()
